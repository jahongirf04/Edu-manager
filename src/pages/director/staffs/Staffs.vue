...
import { useDirectorStaffsStore } from '../../../stores/Director/directorStaffs'
...
const store2 = useDirectorStaffsStore()
...
const count = ref("")
onMounted(async () => {
  const response = await store2.getStaffs(1)
  students.value = response?.data?.staffs
...
const paginate = async (p) => {
  pg.value = p
  const response = await store2.getStaffs(p)
  students.value = response.data.staffs
}
</script>
